...
def residual_PS_error(baseline_table, frequency_range, path, plot = True):
    ...
    for i in range(len(u)):
        residual_covariance = sky_covariance(u[i], 0, frequency_range) + beam_covariance(u[i], v=0, nu=frequency_range)
        model_covariance = sky_covariance(u[i], 0, frequency_range, S_low=1, S_high = 5)

        nu_cov = 2*gain_covariance*model_covariance + (1 + 2*gain_covariance)*residual_covariance
