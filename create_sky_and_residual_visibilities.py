...
def residual_ps_error(baseline_table, frequency_range, residuals ='both', path="./", plot = True):
    ...
    gain_error_covariance = numpy.zeros((len(u), len(frequency_range), len(frequency_range)))

    #Compute all residual to model ratios at different u scales
    for u_index in range(len(u)):
        if residuals == "sky":
            residual_covariance = sky_covariance(u[u_index], 0, frequency_range)
        elif residuals == "beam":
            residual_covariance = beam_covariance(u[u_index], v=0, nu=frequency_range)
        elif residuals == 'both':
            residual_covariance = sky_covariance(u[u_index], 0, frequency_range) + \
                                  beam_covariance(u[u_index], v=0, nu=frequency_range)
    ...
    for i in range(len(u)):
        if residuals == "sky":
            residual_covariance = sky_covariance(u[i], 0, frequency_range)
        elif residuals == "beam":
            residual_covariance = beam_covariance(u[i], v=0, nu=frequency_range)
        elif residuals == 'both':
            residual_covariance = sky_covariance(u[i], 0, frequency_range) + \
                                  beam_covariance(u[i], v=0, nu=frequency_range)

        model_covariance = sky_covariance(u[i], 0, frequency_range, S_low=1, S_high= 5)

        nu_cov = 2*gain_averaged_covariance*model_covariance + (1 + 2*gain_averaged_covariance)*residual_covariance

        cal_variance[i, :] = PS(taper1, taper2, nu_cov, dftmatrix)
